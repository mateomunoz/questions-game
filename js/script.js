...
const showQuestion = () => {
  ...
  //cada opcion de respuesta
  question.options.forEach(option => {
    const optionButton = document.createElement('button');
    optionButton.textContent = option;
    optionButton.classList.add('option-btn');
    //verificar la respuesta al hacer click
...
const checkAnswer = (selectedOption, correctAnswer) => {
  if (selectedOption === correctAnswer) {
    score++;
    //guardar en el array de respuestas correctas
    correctAnswers.push(currentQuestionIndex);
  }
  currentQuestionIndex++;
  //si hay mas preguntas mostrarlas y si no, mostrar el resultado
...
const showResult = () => {
  resultElement.innerHTML = `<p>Result:</p><ul>`;
  QUESTIONS.forEach((question, index) => {
    // crear un elemento de lista para cada pregunta
    const li = document.createElement('li');
    // asignar el texto de la pregunta y la respuesta correspondiente
        ...
        correctAnswers.includes(index)
          ...
          : question.options.indexOf(
              QUESTIONS[index].options.find(
                opt =>
                  opt ===
                  question.options.find((_, i) =>
                    i !== question.options.indexOf(question.correctAnswer) &&
                    correctAnswers.includes(index)
    ...
    //verde si es correcta y rojo si no
    li.style.color = correctAnswers.includes(index) ? 'green' : 'red';
